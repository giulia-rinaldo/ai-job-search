#!/usr/bin/env bun
// Self-contained CLI for searching jobs on jobs.ch, Switzerland's largest job
// board (operated by JobCloud AG). No external CLI framework and zero runtime
// dependencies, so it runs anywhere `bun` is available with just the repo clone.
//
// Personal use only. jobs.ch robots.txt disallows its /api/ endpoints, so this
...
const HELP = `jobs-ch-cli — search jobs on jobs.ch (Switzerland)
...
SEARCH FLAGS
  ...
  --location, -l <text>   Swiss place or canton, e.g. "Zurich", "Genève", "Lausanne", "Ticino".
...
EXAMPLES
  ...
  bun run src/cli.ts search -q "Pflegefachfrau" -l "Bern" --format table
  bun run src/cli.ts search -q "software engineer" -l "Genève" --format json
  bun run src/cli.ts detail 4dd76eb4-17d2-461a-ae8e-bdc64b5dadd2 --format plain

Personal use only — reads jobs.ch public pages; keep volume low (site terms).
`
